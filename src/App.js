...
export const App = () => {
  ...
  const TampilkanData = () => {
    return filmPopuler.map((movie, i) => {
      return (
        <>
          <div className="relative box-border h-80 w-72 overflow-y-scroll  rounded-md bg-amber-50 text-green-100 shadow-xl  lg:h-[25rem] lg:w-64" key={i}>
            <img src={`${gambar}/${movie.poster_path}`} alt="" className="h-[17rem] w-full rounded-t-md lg:h-60 " />
  ...
  return (
    <div>
      <div className=" navbar fixed z-10 flex w-full flex-col items-center justify-center  p-10 shadow-lg  ">
        ...
        <h1 className="text-center text-[1.4em] font-bold uppercase text-primary lg:text-4xl">Find... movie Present</h1>
        <p className="p-2  text-[10px] font-bold text-slate-400">Cari film Favorit anda dengan api terbaik</p>
        <div className="flex items-center justify-center">
          <input type="text" placeholder="Masukan nama film" className="inputa rounded-xl border-[1px] border-teal-200 px-2 text-center lg:px-32 lg:py-2"></input>
